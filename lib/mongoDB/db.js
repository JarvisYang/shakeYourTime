...
var mongoDB = function(){
    ...
    this.findAllAnswerID = function(callback){
        var self = this;
        this.getDB(function(db){
            db.collection('zhihu.answers').find(
                {
                },
                {
                    '_id':1,
    ...
    this.findAnswerByAnswerID = function(answerID,callback){
        var self = this;
        var answerInfo = {};
        this.getDB(function(db){
            db.collection('zhihu.answers').findOne(
                {
                    '_id': answerID
                },
                {
                ...
                function(error,result1){
                    ...
                    if(error){
                        ...
                        self.getDB(function(db){
                            db.collection('zhihu.questions').findOne(
                                {
                                    '_id': answerInfo.questionID
                                },
                                {
                                    'title': 1,
                                    'desc': 1
                                ...
                                function(error,result2){
                                    ...
                                    if(error){
                                        ...
                                        //console.log(result2);
                                        if(!result2){
                                            console.log(answerInfo['_id'],result2)
                                        }
                                        answerInfo.questionTitle = result2.hasOwnProperty('title')?result2.title:null;
                                        answerInfo.questionDesc = result2.hasOwnProperty('desc')?result2.desc:null;
                                        callback(answerInfo);
                                    }
                                })
                        });
    ...
    this.removeAnswerByAnswerID = function(answerID,callback){
        var self = this;
        this.getDB(function(db){
            db.collection('zhihu.answers').remove(
                {
                    '_id': answerID
                },
                function(error,result){
